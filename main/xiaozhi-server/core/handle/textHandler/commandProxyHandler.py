...
- 服务器转发给 ESP32（不重试、不添加 seq_id）
- 支持 app_id 追踪（lock_control 开锁命令缓存到 last_remote_unlock）
"""
import json
...
class LockControlProxyHandler(TextMessageHandler):
    ...
    async def _forward_to_esp32(self, conn, esp32_conn, msg_json: Dict[str, Any]):
        """转发命令到 ESP32（直接转发，不重试）"""
        try:
            command = msg_json.get("command")

            # 开锁命令：缓存 app_id 到 ESP32 连接对象（供 logReportHandler 使用）
            if command == "unlock":
            ...
            conn.logger.bind(tag=TAG).debug(f"命令已转发到 ESP32: command={command}")
...
class DevControlProxyHandler(TextMessageHandler):
    ...
    async def _forward_to_esp32(self, conn, esp32_conn, msg_json: Dict[str, Any]):
        ...
        try:
            await esp32_conn.websocket.send(json.dumps(msg_json, ensure_ascii=False))
            conn.logger.bind(tag=TAG).debug(f"设备控制命令已转发: target={msg_json.get('target')}")
        except Exception as e:
            conn.logger.bind(tag=TAG).error(f"转发命令失败: {e}")
            await self._send_error(conn, f"转发失败: {str(e)}")
...
class UserMgmtProxyHandler(TextMessageHandler):
    ...
    async def _forward_to_esp32(self, conn, esp32_conn, msg_json: Dict[str, Any]):
        ...
        注意：user_name 是 App 专用字段，转发前剥离，不发送给 ESP32。
        Server 负责将 user_name 存入数据库关联 user_id。
        """
        try:
            # 去掉 App 专用字段再转发
            user_name = msg_json.pop("user_name", None)
            if user_name:
                conn.logger.bind(tag=TAG).debug(
                    f"已剥离 user_name 字段: {user_name}, user_id={msg_json.get('user_id')}"
                )

            await esp32_conn.websocket.send(json.dumps(msg_json, ensure_ascii=False))
            conn.logger.bind(tag=TAG).debug(
                f"用户管理命令已转发: category={msg_json.get('category')}, command={msg_json.get('command')}"
            )
        except Exception as e:
            conn.logger.bind(tag=TAG).error(f"转发命令失败: {e}")
            await self._send_error(conn, f"转发失败: {str(e)}")
